import TResEntityDestroy from "./lib/networking/tcp/response/TResEntityDestroy";
import EPacketChannel from "./lib/enums/EPacketChannel";
import TResPlayerCreate from "./lib/networking/tcp/response/TResPlayerCreate";
import TResProjectileCreate from "./lib/networking/tcp/response/TResProjectileCreate";

function logBufferInBinary(buffer: Buffer) {
    for (let i = 0; i < buffer.length; i++) {
        const binaryRepresentation = buffer[i].toString(2).padStart(8, '0');
        console.log(binaryRepresentation);
    }
}

const packet = new TResProjectileCreate()
packet.channel = EPacketChannel.UDP;
packet.hasWeight = 1;
packet.bounce = 1;
packet.dieOnCollision = 1;
packet.collision = 1;
packet.bounceFriction = 0.48;
packet.direction = 3200;
packet.x = 3232000;

const buff = packet.bake();
console.log(buff.getBuffer());
